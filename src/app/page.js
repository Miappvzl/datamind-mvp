    ...
    <div className="flex min-h-screen flex-col items-center p-6 bg-slate-950 text-white font-sans">
      ...
      {!result && !loading && (
          <div className="w-full max-w-xl h-64 border-2 border-dashed border-slate-700 rounded-2xl flex flex-col items-center justify-center text-center hover:border-emerald-500 hover:bg-slate-900/50 transition-all cursor-pointer relative group bg-slate-900/20">
            ...
            <p className="text-slate-300 font-medium text-lg">Tocar para subir foto</p>
            <p className="text-slate-500 text-xs mt-2 px-4">La IA optimizará y leerá tu imagen automáticamente</p>
          </div>
      )}
      ...
      {loading && (
          <div className="flex flex-col items-center justify-center h-64 animate-pulse">
              ...
              <p className="text-blue-400 font-mono text-lg">Procesando imagen...</p>
              <p className="text-slate-500 text-xs mt-2">Comprimiendo y enviando a Gemini AI</p>
          </div>
      )}
        ...
        <div className="grid grid-cols-1 gap-6 w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl relative overflow-hidden">
                ...
                <div className="mb-6 pr-8">
                    <span className="text-slate-500 text-[10px] uppercase tracking-wider font-bold">Entidad / Nombre</span>
                    <h2 className="text-xl md:text-2xl font-bold text-white mt-1 leading-tight">{result.entidad_nombre}</h2>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                        <span className="text-slate-500 text-[10px] uppercase tracking-wider block mb-1">Documento</span>
                        <p className="text-base font-mono text-blue-300 truncate">{result.numero_documento}</p>
                    </div>
                    <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                        <span className="text-slate-500 text-[10px] uppercase tracking-wider block mb-1">Fecha</span>
                ...
                {result.tipo_documento === 'factura' && (
                    <div className="mt-4 pt-4 border-t border-dashed border-slate-800">
                        <div className="mb-4">
                             <span className="text-slate-500 text-[10px] uppercase tracking-wider">Cliente</span>
                             <p className="text-slate-300 text-sm font-medium">{result.cliente_nombre || "Consumidor Final"}</p>
                        </div>
                        <div className="flex flex-col bg-emerald-900/10 p-4 rounded-xl border border-emerald-500/20 text-center">
                            <span className="text-emerald-500 text-xs uppercase font-bold mb-1">Total a Pagar</span>
                            <p className="text-emerald-400 text-3xl font-bold tracking-tight">{result.monto_total}</p>
                            <p className="text-slate-500 text-xs mt-2 border-t border-emerald-500/10 pt-2">{result.detalles_extra}</p>
                        </div>
                    </div>
                )}
          </div>
